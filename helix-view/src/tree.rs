...
impl Tree {
    ...
    pub fn get(&self, index: ViewId) -> &View {
        // Distinguish the two failure modes up front so future crashes
        // (e.g. the `hx .` startup path that called this with a stale /
        // container id) report which case they hit instead of landing
        // on a bare `unwrap() on a None value`.
