...
(provide syntax_symbol_picker)
;;@doc
;;Open symbol picker from syntax information
(define (syntax_symbol_picker)
    (helix.static.syntax_symbol_picker *helix.cx*))

(provide lsp_or_syntax_symbol_picker)
;;@doc
;;Open symbol picker from LSP or syntax information
(define (lsp_or_syntax_symbol_picker)
    (helix.static.lsp_or_syntax_symbol_picker *helix.cx*))
...
(provide syntax_workspace_symbol_picker)
;;@doc
;;Open workspace symbol picker from syntax information
(define (syntax_workspace_symbol_picker)
    (helix.static.syntax_workspace_symbol_picker *helix.cx*))

(provide lsp_or_syntax_workspace_symbol_picker)
;;@doc
;;Open workspace symbol picker from LSP or syntax information
(define (lsp_or_syntax_workspace_symbol_picker)
    (helix.static.lsp_or_syntax_workspace_symbol_picker *helix.cx*))
...
;;Goto column
(define (goto_column)
    (helix.static.goto_column *helix.cx*))

(provide extend_to_column)
;;@doc
;;Extend to column
(define (extend_to_column)
    (helix.static.extend_to_column *helix.cx*))
...
(provide insert_char_interactive)
;;@doc
;;Insert an interactively-chosen char
(define (insert_char_interactive)
    (helix.static.insert_char_interactive *helix.cx*))

(provide append_char_interactive)
;;@doc
;;Append an interactively-chosen char
(define (append_char_interactive)
    (helix.static.append_char_interactive *helix.cx*))
...
(provide goto_next_xml_element)
;;@doc
;;Goto next (X)HTML element
(define (goto_next_xml_element)
    (helix.static.goto_next_xml_element *helix.cx*))

(provide goto_prev_xml_element)
;;@doc
;;Goto previous (X)HTML element
(define (goto_prev_xml_element)
    (helix.static.goto_prev_xml_element *helix.cx*))
...
(provide rotate_selections_first)
;;@doc
;;Make the first selection your primary one
(define (rotate_selections_first)
    (helix.static.rotate_selections_first *helix.cx*))

(provide rotate_selections_last)
;;@doc
;;Make the last selection your primary one
(define (rotate_selections_last)
    (helix.static.rotate_selections_last *helix.cx*))
...
(define (push-range-to-selection! arg)
    (helix.static.push-range-to-selection! *helix.cx* arg))

(provide set-current-selection-primary-index!)
;;@doc
;;Set the primary index of the current selection
(define (set-current-selection-primary-index! arg)
    (helix.static.set-current-selection-primary-index! *helix.cx* arg))

(provide remove-current-selection-range!)
;;@doc
;;Remove a range from the current selection
(define (remove-current-selection-range! arg)
    (helix.static.remove-current-selection-range! *helix.cx* arg))
...
(define (get-current-line-character arg)
    (helix.static.get-current-line-character *helix.cx* arg))
...
;;@doc
;;Returns the current selection as a string
(define (current-selection->string)
    (helix.static.current-selection->string *helix.cx*))
...
(define (get-current-column-number)
    (helix.static.get-current-column-number *helix.cx*))
...
(define (commit-changes-to-history)
    (helix.static.commit-changes-to-history *helix.cx*))

(provide add-raw-content!)
;;@doc
...
(define (add-raw-content! arg1 arg2 arg3 arg4)
    (helix.static.add-raw-content! *helix.cx* arg1 arg2 arg3 arg4))

(provide add-raw-content-with-placeholders!)
;;@doc
...
(define (add-raw-content-with-placeholders! arg1 arg2 arg3 arg4 arg5)
    (helix.static.add-raw-content-with-placeholders! *helix.cx* arg1 arg2 arg3 arg4 arg5))
